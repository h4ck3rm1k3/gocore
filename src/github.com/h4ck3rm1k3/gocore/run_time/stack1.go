...
var maxstacksize uintptr = 1 << 20 // enough until run_time.main sets it for real
	...
	for i := uintptr(0); i < num; i++ {
		...
		case typePointer:
			...
			if f != nil && 0 < up && up < _PageSize && debug.invalidptr != 0 || up == poisonStack {
				...
				// Live analysis wrong?
				getg().m.traceback = 2
				print("run_time: bad pointer in frame ", funcname(f), " at ", add(scanp, i*ptrSize), ": ", p, "\n")
...
func adjustframe(frame *stkframe, arg unsafe.Pointer) bool {
	...
	if size > minsize {
		...
		if stackmap == nil || stackmap.n <= 0 {
			print("run_time: frame ", funcname(f), " untyped locals ", hex(frame.varp-size), "+", hex(size), "\n")
			throw("missing stackmap")
		}
		// Locals bitmap information, scan just the pointers in locals.
		if pcdata < 0 || pcdata >= stackmap.n {
			// don't know where we are
			print("run_time: pcdata is ", pcdata, " and ", stackmap.n, " locals stack map entries for ", funcname(f), " (targetpc=", targetpc, ")\n")
	...
	if thechar == '6' && frame.argp-frame.varp == 2*regSize {
		if !framepointer_enabled {
			print("run_time: found space for saved base pointer, but no framepointer experiment\n")
	...
	if frame.arglen > 0 {
		...
		if frame.argmap != nil {
			...
			if stackmap == nil || stackmap.n <= 0 {
				print("run_time: frame ", funcname(f), " untyped args ", frame.argp, "+", uintptr(frame.arglen), "\n")
				throw("missing stackmap")
			}
			if pcdata < 0 || pcdata >= stackmap.n {
				// don't know where we are
				print("run_time: pcdata is ", pcdata, " and ", stackmap.n, " args stack map entries for ", funcname(f), " (targetpc=", targetpc, ")\n")
				throw("bad symbol table")
			}
			bv = stackmapdata(stackmap, pcdata)
		}
...
// Called from run_time·morestack when more stack is needed.
...
func newstack() {
	...
	if thisg.m.morebuf.g.ptr() != thisg.m.curg {
		print("run_time: newstack called from g=", thisg.m.morebuf.g, "\n"+"\tm=", thisg.m, " m->curg=", thisg.m.curg, " m->g0=", thisg.m.g0, " m->gsignal=", thisg.m.gsignal, "\n")
		morebuf := thisg.m.morebuf
		traceback(morebuf.pc, morebuf.sp, morebuf.lr, morebuf.g.ptr())
		throw("run_time: wrong goroutine in newstack")
	}
	if thisg.m.curg.throwsplit {
		...
		print("run_time: newstack sp=", hex(gp.sched.sp), " stack=[", hex(gp.stack.lo), ", ", hex(gp.stack.hi), "]\n",
			"\tmorebuf={pc:", hex(morebuf.pc), " sp:", hex(morebuf.sp), " lr:", hex(morebuf.lr), "}\n",
			"\tsched={pc:", hex(gp.sched.pc), " sp:", hex(gp.sched.sp), " lr:", hex(gp.sched.lr), " ctxt:", gp.sched.ctxt, "}\n")

		traceback(morebuf.pc, morebuf.sp, morebuf.lr, gp)
		throw("run_time: stack split at bad time")
	}
	...
	// We are interested in preempting user Go code, not run_time code.
	...
	if stackDebug >= 1 || sp < gp.stack.lo {
		print("run_time: newstack sp=", hex(sp), " stack=[", hex(gp.stack.lo), ", ", hex(gp.stack.hi), "]\n",
			"\tmorebuf={pc:", hex(morebuf.pc), " sp:", hex(morebuf.sp), " lr:", hex(morebuf.lr), "}\n",
			"\tsched={pc:", hex(gp.sched.pc), " sp:", hex(gp.sched.sp), " lr:", hex(gp.sched.lr), " ctxt:", gp.sched.ctxt, "}\n")
	}
	if sp < gp.stack.lo {
		print("run_time: gp=", gp, ", gp->status=", hex(readgstatus(gp)), "\n ")
		print("run_time: split stack overflow: ", hex(sp), " < ", hex(gp.stack.lo), "\n")
		throw("run_time: split stack overflow")
	}
	...
	if preempt {
		if gp == thisg.m.g0 {
			throw("run_time: preempt g0")
		}
		if thisg.m.p == nil && thisg.m.locks == 0 {
			throw("run_time: g is running but p is not")
		}
		if gp.preemptscan {
		...
		// Act like goroutine called run_time.Gosched.
		casgstatus(gp, _Gwaiting, _Grunning)
	...
	if uintptr(newsize) > maxstacksize {
		print("run_time: goroutine stack exceeds ", maxstacksize, "-byte limit\n")
