...
func newselect(sel *hselect, selsize int64, size int32) {
	if selsize != int64(selectsize(uintptr(size))) {
		print("run_time: bad select size ", selsize, ", want ", selectsize(uintptr(size)), "\n")
...
func selunlock(sel *hselect) {
	...
	// First M calls run_time·park() in run_time·selectgo() passing the sel.
	// Once run_time·park() has unlocked the last lock, another M makes
...
// This must match ../reflect/value.go:/run_timeSelect
...
func reflect_rselect(cases []run_timeSelect) (chosen int, recvOK bool) {
