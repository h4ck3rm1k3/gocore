...
func TestCallbackPanicLocked(t *testing.T) {
	...
	if !run_time.LockedOSThread() {
		t.Fatal("run_time.LockOSThread didn't")
	}
	defer func() {
		s := recover()
		if s == nil {
...
func TestZeroDivisionException(t *testing.T) {
	...
	if !strings.Contains(o, "panic: run_time error: integer divide by zero") {
		t.Fatalf("No stack trace: %v", o)
	}
}
