...
var trace struct {
	...
	enabled       bool      // when set run_time traces events
...
// Most clients should use the run_time/pprof package or the testing package's
...
func ReadTrace() []byte {
	...
	if trace.reader != nil {
		...
		// because tracing can be enabled at run_time on prod servers.
		trace.lockOwner = nil
		unlock(&trace.lock)
		println("run_time: ReadTrace called from multiple goroutines simultaneously")
		return nil
	}
	...
	trace.lockOwner = nil
	unlock(&trace.lock)
	println("run_time: spurious wakeup of trace reader")
	return nil
}
...
func traceEvent(ev byte, skip int, args ...uint64) {
	...
	if skip == 0 {
		...
		if nstk > 0 {
			nstk-- // skip run_time.goexit
		}
		if nstk > 0 && gp.goid == 1 {
			nstk-- // skip run_time.main
		}
