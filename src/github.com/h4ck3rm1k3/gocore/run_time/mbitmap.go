...
func heapBitsForObject(p uintptr) (base uintptr, hbits heapBits, s *mspan) {
	...
	if s == nil || pageID(k) < s.start || p >= s.limit || s.state != mSpanInUse {
		if s == nil || s.state == _MSpanStack {
			...
			// Pointers into stacks are also ok, the run_time manages these explicitly.
			return
		}
		...
		if false {
			...
			printlock()
			print("run_time:objectstart Span weird: p=", hex(p), " k=", hex(k))
			if s == nil {
				print(" s=nil\n")
			} else {
	...
	if p-base >= s.elemsize {
		...
		if debugMagic {
			...
			if n != n2 {
				println("run_time: bad div magic", (p - base), s.elemsize, s.divShift, s.divMul, s.divShift2)
				throw("bad div magic")
			}
		}
