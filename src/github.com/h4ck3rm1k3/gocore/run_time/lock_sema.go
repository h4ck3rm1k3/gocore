...
//
//	uintptr run_time·semacreate(void)
//		Create a semaphore, which will be assigned to m->waitsema.
//		The zero value is treated as absence of any semaphore,
//		so be sure to return a non-zero value.
//
//	int32 run_time·semasleep(int64 ns)
//		If ns < 0, acquire m->waitsema and return 0.
//		If ns >= 0, try to acquire m->waitsema for at most ns nanoseconds.
//		Return 0 if the semaphore was acquired, -1 if interrupted or timed out.
//
//	int32 run_time·semawakeup(M *mp)
...
func lock(l *mutex) {
	gp := getg()
	if gp.m.locks < 0 {
		throw("run_time·lock: lock count")
	}
	gp.m.locks++
...
func unlock(l *mutex) {
	...
	gp.m.locks--
	if gp.m.locks < 0 {
		throw("run_time·unlock: lock count")
	}
...
func notetsleep_internal(n *note, ns int64, gp *g, deadline int64) bool {
	...
	for {
		...
		case locked:
			...
			gp.m.blocked = true
			if semasleep(-1) < 0 {
				throw("run_time: unable to acquire - semaphore out of sync")
			}
			gp.m.blocked = false
			return true
		default:
			throw("run_time: unexpected waitm - semaphore out of sync")
		}
	}
}
...
// same as run_time·notetsleep, but called on user g (not g0)
