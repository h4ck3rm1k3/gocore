...
func minit() {
	...
	if ret < 0 {
		print("run_time: nacl_exception_stack: error ", -ret, "\n")
	}

	ret = nacl_exception_handler(funcPC(sigtramp), nil)
	if ret < 0 {
		print("run_time: nacl_exception_handler: error ", -ret, "\n")
	}
}
...
var badsignal1 = []byte("run_time: signal received on thread not created by Go.\n")
