...
type pollDesc struct {
	...
	// pollReset, pollWait, pollWaitCanceled and run_time·netpollready (IO readiness notification)
...
//go:linkname net_run_time_pollServerInit net.run_time_pollServerInit
func net_run_time_pollServerInit() {
	netpollinit()
...
//go:linkname net_run_time_pollOpen net.run_time_pollOpen
func net_run_time_pollOpen(fd uintptr) (*pollDesc, int) {
	pd := pollcache.alloc()
	lock(&pd.lock)
...
//go:linkname net_run_time_pollClose net.run_time_pollClose
func net_run_time_pollClose(pd *pollDesc) {
	if !pd.closing {
...
//go:linkname net_run_time_pollReset net.run_time_pollReset
func net_run_time_pollReset(pd *pollDesc, mode int) int {
...
//go:linkname net_run_time_pollWait net.run_time_pollWait
func net_run_time_pollWait(pd *pollDesc, mode int) int {
	err := netpollcheckerr(pd, int32(mode))
	if err != 0 {
		return err
	}
...
//go:linkname net_run_time_pollWaitCanceled net.run_time_pollWaitCanceled
func net_run_time_pollWaitCanceled(pd *pollDesc, mode int) {
...
//go:linkname net_run_time_pollSetDeadline net.run_time_pollSetDeadline
func net_run_time_pollSetDeadline(pd *pollDesc, d int64, mode int) {
	lock(&pd.lock)
	if pd.closing {
		unlock(&pd.lock)
		return
...
//go:linkname net_run_time_pollUnblock net.run_time_pollUnblock
func net_run_time_pollUnblock(pd *pollDesc) {
	lock(&pd.lock)
	if pd.closing {
...
func netpollblock(pd *pollDesc, mode int32, waitio bool) bool {
	...
	// this is necessary because run_time_pollUnblock/run_time_pollSetDeadline/deadlineimpl
...
func netpollunblock(pd *pollDesc, mode int32, ioready bool) *g {
	...
	for {
		...
		if old == 0 && !ioready {
			// Only set READY for ioready. run_time_pollWait
