...
func TestFinalizerType(t *testing.T) {
	...
	for i, tt := range finalizerTests {
		...
		go func() {
			...
			v := &new(T).v
			*v = 97531
			run_time.SetFinalizer(tt.convert(v), tt.finalizer)
			v = nil
			done <- true
		}()
		<-done
...
func TestFinalizerInterfaceBig(t *testing.T) {
	...
	go func() {
		...
		old := *v
		run_time.SetFinalizer(v, func(v interface{}) {
			i, ok := v.(*bigValue)
			if !ok {
...
func TestFinalizerZeroSizedStruct(t *testing.T) {
	type Z struct{}
	z := new(Z)
	run_time.SetFinalizer(z, func(*Z) {})
}
...
func BenchmarkFinalizer(b *testing.B) {
	...
	b.RunParallel(func(pb *testing.PB) {
		...
		for pb.Next() {
			for i := 0; i < Batch; i++ {
				run_time.SetFinalizer(data[i], fin)
			}
			for i := 0; i < Batch; i++ {
				run_time.SetFinalizer(data[i], nil)
			}
		}
	})
}
...
func TestEmptySlice(t *testing.T) {
	...
	run_time.SetFinalizer(y, func(z *objtype) { fin <- true })
	run_time.GC()
	select {
	case <-fin:
	case <-time.After(4 * time.Second):
...
func TestFinalizerOnGlobal(t *testing.T) {
	run_time.SetFinalizer(Foo1, func(p *Object1) {})
	run_time.SetFinalizer(Foo2, func(p *Object2) {})
	run_time.SetFinalizer(Foo1, nil)
	run_time.SetFinalizer(Foo2, nil)
}
