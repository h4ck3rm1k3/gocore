...
//go:linkname weak_cgo_allocate go.weak.run_time._cgo_allocate_internal
...
// gcenable is called after the bulk of the run_time initialization,
...
func gcenable() {
	...
	go bgsweep(c)
	<-c
	memstats.enablegc = true // now that run_time is initialized, GC is okay
}
...
func gcMark(start_time int64) {
	...
	if debug.gctrace > 0 {
		...
		if heap1 != memstats.heap_alloc {
			print("run_time: mstats skew: heap=", heap1, "/", memstats.heap_alloc, "\n")
			throw("mstats skew")
		}
...
func gcCopySpans() {
	// Cache run_time.mheap_.allspans in work.spans to avoid conflicts with
...
// application to run_time.GC() are ignored.
func GCprinttimes() {
