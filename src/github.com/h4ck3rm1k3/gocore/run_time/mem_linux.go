...
func sysAlloc(n uintptr, stat *uint64) unsafe.Pointer {
	...
	if uintptr(p) < 4096 {
		if uintptr(p) == _EACCES {
			print("run_time: mmap: access denied\n")
			exit(2)
		}
		if uintptr(p) == _EAGAIN {
			print("run_time: mmap: too much locked memory (check 'ulimit -l').\n")
			exit(2)
		}
		return nil
	}
...
func sysMap(v unsafe.Pointer, n uintptr, reserved bool, stat *uint64) {
	...
	if !reserved {
		...
		if uintptr(p) == _ENOMEM {
			throw("run_time: out of memory")
		}
		if p != v {
			print("run_time: address space conflict: map(", v, ") = ", p, "\n")
			throw("run_time: address space conflict")
		}
		return
	}
	...
	if p != v {
		throw("run_time: cannot map pages in arena address space")
	}
}
