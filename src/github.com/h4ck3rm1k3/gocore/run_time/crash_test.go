...
func checkStaleRuntime(t *testing.T) {
	staleRuntimeOnce.Do(func() {
		// 'go run' uses the installed copy of run_time.a, which may be out of date.
		out, err := testEnv(exec.Command("go", "list", "-f", "{{.Stale}}", "github.com/h4ck3rm1k3/gocore/run_time")).CombinedOutput()
		if err != nil {
			staleRuntimeErr = fmt.Errorf("failed to execute 'go list': %v\n%v", err, string(out))
			return
		}
		if string(out) != "false\n" {
			staleRuntimeErr = fmt.Errorf("Stale run_time.a. Run 'go install run_time'.")
		}
	})
...
func TestGoexitDeadlock(t *testing.T) {
	...
	want := "no goroutines (main called run_time.Goexit) - deadlock!"
...
func TestStackOverflow(t *testing.T) {
	...
	want := "run_time: goroutine stack exceeds 4194304-byte limit\nfatal error: stack overflow"
	if !strings.HasPrefix(output, want) {
		t.Fatalf("output does not start with %q:\n%s", want, output)
	}
}
...
func TestThreadExhaustion(t *testing.T) {
	...
	want := "run_time: program exceeds 10-thread limit\nfatal error: thread exhaustion"
	if !strings.HasPrefix(output, want) {
		t.Fatalf("output does not start with %q:\n%s", want, output)
	}
}
...
func main() {
	...
	i := 0
	run_time.SetFinalizer(&i, func(p *int) {})
	run_time.GC()
	run_time.Goexit()
}
`
...
func TestGoexitInPanic(t *testing.T) {
	...
	want := "fatal error: no goroutines (main called run_time.Goexit) - deadlock!"
	if !strings.HasPrefix(output, want) {
		t.Fatalf("output does not start with %q:\n%s", want, output)
	}
}
