...
func symtabinit() {
	...
	if pcln32[0] != 0xfffffffb || pcln[4] != 0 || pcln[5] != 0 || pcln[6] != _PCQuantum || pcln[7] != ptrSize {
		println("run_time: function symbol table header:", hex(pcln32[0]), hex(pcln[4]), hex(pcln[5]), hex(pcln[6]), hex(pcln[7]))
	...
	for i := 0; i < nftab; i++ {
		...
		if ftab[i].entry > ftab[i+1].entry {
			...
			throw("invalid run_time symbol table")
		}
	}
...
func pcvalue(f *_func, off int32, targetpc uintptr, strict bool) int32 {
	...
	print("run_time: invalid pc-encoded table f=", funcname(f), " pc=", hex(pc), " targetpc=", hex(targetpc), " tab=", p, "\n")
...
func funcdata(f *_func, i int32) unsafe.Pointer {
	...
	if ptrSize == 8 && uintptr(p)&4 != 0 {
		if uintptr(unsafe.Pointer(f))&4 != 0 {
			println("run_time: misaligned func", f)
		}
		p = add(p, 4)
	}
