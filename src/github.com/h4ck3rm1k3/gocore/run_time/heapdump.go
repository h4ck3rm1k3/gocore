...
// Implementation of run_time/debug.WriteHeapDump.  Writes all
...
//go:linkname run_time_debug_WriteHeapDump run_time/debug.WriteHeapDump
func run_time_debug_WriteHeapDump(fd uintptr) {
...
func dumpgs() {
	...
	for i := 0; uintptr(i) < allglen; i++ {
		...
		switch status {
		default:
			print("run_time: unexpected G.status ", hex(status), "\n")
...
func dumpobjs() {
	for i := uintptr(0); i < uintptr(mheap_.nspan); i++ {
		...
		for l := s.freelist; gclinkptr(l).ptr() != nil; l = uintptr(gclinkptr(l).ptr().next) {
