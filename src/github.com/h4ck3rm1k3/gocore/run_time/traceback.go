...
func tracebackinit() {
	// Go variable initialization happens late during run_time startup.
...
func tracebackdefers(gp *g, callback func(*stkframe, unsafe.Pointer) bool, v unsafe.Pointer) {
	...
	for d := gp._defer; d != nil; d = d.link {
		...
		if fn == nil {
			...
			f := findfunc(frame.pc)
			if f == nil {
				print("run_time: unknown pc in defer ", hex(frame.pc), "\n")
				throw("unknown pc")
			}
			frame.fn = f
...
// Generic traceback.  Handles run_time stack prints (pcbuf == nil),
// the run_time.Callers function (pcbuf != nil), as well as the garbage
...
func gentraceback(pc0, sp0, lr0 uintptr, gp *g, skip int, pcbuf *uintptr, max int, callback func(*stkframe, unsafe.Pointer) bool, v unsafe.Pointer, flags uint) int {
	...
	if f == nil {
		if callback != nil {
			print("run_time: unknown pc ", hex(frame.pc), "\n")
			throw("unknown pc")
		}
		return 0
	}
	...
	for n < max {
		...
		if topofstack(f) {
			...
			if flr == nil {
				...
				if callback != nil {
					print("run_time: unexpected return pc for ", funcname(f), " called from ", hex(frame.lr), "\n")
		...
		// in package run_time and reflect, and for those we use call-specific
		...
		if printing {
			if (flags&_TraceRuntimeFrames) != 0 || showframe(f, gp) {
				// Print during crash.
				//	main(0x1, 0x2, 0x3)
				//		/home/rsc/go/src/run_time/x.go:23 +0xf
				//
	...
	if callback != nil && n < max && _defer != nil {
		if _defer != nil {
			print("run_time: g", gp.goid, ": leftover defer sp=", hex(_defer.sp), " pc=", hex(_defer.pc), "\n")
		}
...
func setArgInfo(frame *stkframe, f *_func, needArgMap bool) {
	...
	if needArgMap && f.args == _ArgsSizeUnknown {
		...
		case "reflect.makeFuncStub", "reflect.methodValueCall":
			...
			if fn[0] != f.entry {
				print("run_time: confused by ", funcname(f), "\n")
...
func traceback1(pc, sp, lr uintptr, gp *g, flags uint) {
	...
	// Print traceback. By default, omits run_time frames.
...
func showframe(f *_func, gp *g) bool {
	...
	// Special case: always show run_time.panic frame, so that we can
	// see where a panic started in the middle of a stack trace.
	// See golang.org/issue/5832.
	if name == "run_time.panic" {
		return true
	}

	return traceback > 1 || f != nil && contains(name, ".") && (!hasprefix(name, "run_time.") || isExportedRuntime(name))
}

// isExportedRuntime reports whether name is an exported run_time function.
// It is only for run_time functions, so ASCII A-Z is fine.
func isExportedRuntime(name string) bool {
	const n = len("run_time.")
	return len(name) > n && name[:n] == "run_time." && 'A' <= name[n] && name[n] <= 'Z'
}
...
var gStatusStrings = [...]string{
	...
	_Grunnable:  "runnable",
	_Grunning:   "running",
	_Gsyscall:   "github.com/h4ck3rm1k3/gocore/syscall",
	_Gwaiting:   "waiting",
	_Gdead:      "dead",
	_Genqueue:   "enqueue",
