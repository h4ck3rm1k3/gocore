...
type g struct {
	...
	stack       stack   // offset known to run_time/cgo
...
type p struct {
	...
	// Cache of goroutine ids, amortizes accesses to run_time·sched.goidgen.
	goidcache    uint64
	goidcacheend uint64
...
const (
	...
	_SigGoExit   = 1 << 7 // cause all run_time procs to exit (only used on Plan 9).
...
// and with package debug/gosym and with symtab.go in package run_time.
type _func struct {
...
const (
	_TraceRuntimeFrames = 1 << 0 // include frames for internal run_time functions.
 ...
 */
// bool	run_time·notetsleep(Note*, int64);  // false - timeout
// bool	run_time·notetsleepg(Note*, int64);  // false - timeout
