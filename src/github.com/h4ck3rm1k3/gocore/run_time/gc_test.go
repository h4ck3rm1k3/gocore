...
func main() {
	...
	memstats := new(run_time.MemStats)
	run_time.GC()
	run_time.ReadMemStats(memstats)
	sys := memstats.Sys

	run_time.MemProfileRate = 0 // disable profiler
...
func TestGcLastTime(t *testing.T) {
	ms := new(run_time.MemStats)
	t0 := time.Now().UnixNano()
	run_time.GC()
	t1 := time.Now().UnixNano()
	run_time.ReadMemStats(ms)
...
func TestHugeGCInfo(t *testing.T) {
	...
	// The types are not allocated at run_time.
	if hugeSink != nil {
