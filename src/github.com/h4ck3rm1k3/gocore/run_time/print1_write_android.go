...
func initLegacy() {
	...
	if writeFD == 0 {
		...
		msg := []byte("run_time: cannot open /dev/log/main\x00")
...
func initLogd() {
	...
	if fd < 0 {
		msg := []byte("run_time: cannot create a socket for logging\x00")
		write(2, unsafe.Pointer(&msg[0]), int32(len(msg)))
		exit(2)
	}
	...
	if errno < 0 {
		msg := []byte("run_time: cannot connect to /dev/socket/logdw\x00")
		write(2, unsafe.Pointer(&msg[0]), int32(len(msg)))
