...
//go:cgo_export_dynamic run_time.end _end
//go:cgo_export_dynamic run_time.etext _etext
//go:cgo_export_dynamic run_time.edata _edata
...
func newosproc(mp *m, _ unsafe.Pointer) {
	...
	if ret != 0 {
		print("run_time: failed to create new OS thread (have ", mcount(), " already; errno=", ret, ")\n")
		throw("newosproc")
	}
}
...
func memlimit() uintptr {
	/*
		...
		Rlimit rl;
		extern byte run_time·text[], run_time·end[];
		uintptr used;

		if(run_time·getrlimit(RLIMIT_AS, &rl) != 0)
			return 0;
		...
		used = run_time·end - run_time·text + (64<<20);
		if(used >= rl.rlim_cur)
			return 0;
