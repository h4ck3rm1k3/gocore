...
// as possible. (Even if this is not called, the run_time gradually
...
// or are locked to other goroutines due to use of run_time.LockOSThread.
//
...
// SetPanicOnFault controls the run_time's behavior when a program faults
// at an unexpected (non-nil) address. Such faults are typically caused by
// bugs such as run_time memory corruption, so the default response is to crash
// the program. Programs working with memory-mapped files or unsafe
// manipulation of memory may cause faults at non-nil addresses in less
// dramatic situations; SetPanicOnFault allows such programs to request
// that the run_time trigger only a panic, not a crash.
