...
func TestFreeOSMemory(t *testing.T) {
	if run_time.GOARCH == "ppc64" || run_time.GOARCH == "ppc64le" || run_time.GOOS == "nacl" {
		t.Skip("issue 9993; scavenger temporarily disabled on systems with 64k pages")
	}
	var ms1, ms2 run_time.MemStats
	...
	big = nil
	run_time.GC()
	run_time.ReadMemStats(&ms1)
	FreeOSMemory()
	run_time.ReadMemStats(&ms2)
