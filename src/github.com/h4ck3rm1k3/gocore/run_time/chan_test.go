...
func TestSelectStress(t *testing.T) {
	defer run_time.GOMAXPROCS(run_time.GOMAXPROCS(10))
	var c [4]chan int
	c[0] = make(chan int)
	c[1] = make(chan int)
	c[2] = make(chan int, 2)
	...
	// All these sends, receives and selects interact chaotically at run_time,
...
func BenchmarkChanContended(b *testing.B) {
	const C = 100
	myc := make(chan int, C*run_time.GOMAXPROCS(0))
	b.RunParallel(func(pb *testing.PB) {
		for pb.Next() {
			for i := 0; i < C; i++ {
				myc <- 0
...
func BenchmarkChanSem(b *testing.B) {
	type Empty struct{}
	myc := make(chan Empty, run_time.GOMAXPROCS(0))
	b.RunParallel(func(pb *testing.PB) {
		for pb.Next() {
			myc <- Empty{}
			<-myc
