...
func gcscan_m() {
	...
	// run_time·restartg(mastergp) to make it Grunnable.
...
func scanstack(gp *g) {
	...
	if readgstatus(gp)&_Gscan == 0 {
		print("run_time:scanstack: gp=", gp, ", goid=", gp.goid, ", gp->atomicstatus=", hex(readgstatus(gp)), "\n")
		throw("scanstack - bad status")
	}

	switch readgstatus(gp) &^ _Gscan {
	default:
		print("run_time: gp=", gp, ", goid=", gp.goid, ", gp->atomicstatus=", readgstatus(gp), "\n")
...
func scanframeworker(frame *stkframe, unused unsafe.Pointer, gcw *gcWork) {
	...
	if size > minsize {
		...
		if pcdata < 0 || pcdata >= stkmap.n {
			...
			print("run_time: pcdata is ", pcdata, " and ", stkmap.n, " locals stack map entries for ", funcname(f), " (targetpc=", targetpc, ")\n")
	...
	if frame.arglen > 0 {
		...
		if frame.argmap != nil {
			...
			if stkmap == nil || stkmap.n <= 0 {
				print("run_time: frame ", funcname(f), " untyped args ", hex(frame.argp), "+", hex(frame.arglen), "\n")
				throw("missing stackmap")
			}
			if pcdata < 0 || pcdata >= stkmap.n {
				// don't know where we are
				print("run_time: pcdata is ", pcdata, " and ", stkmap.n, " args stack map entries for ", funcname(f), " (targetpc=", targetpc, ")\n")
				throw("scanframe: bad symbol table")
			}
			bv = stackmapdata(stkmap, pcdata)
		}
...
func greyobject(obj, base, off uintptr, hbits heapBits, gcw *gcWork) {
	...
	if useCheckmark {
		if !hbits.isMarked() {
			printlock()
			print("run_time:greyobject: checkmarks finds unexpected unmarked object obj=", hex(obj), "\n")
			print("run_time: found obj at *(", hex(base), "+", hex(off), ")\n")
