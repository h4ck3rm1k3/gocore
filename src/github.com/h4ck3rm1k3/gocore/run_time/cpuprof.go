...
//
// Most clients should use the run_time/pprof package or
...
func SetCPUProfileRate(hz int) {
	...
	if hz > 0 {
		if cpuprof == nil {
			...
			if cpuprof == nil {
				print("run_time: cpu profiling cannot allocate memory\n")
				unlock(&cpuprofLock)
				return
			}
		}
		if cpuprof.on || cpuprof.handoff != 0 {
			print("run_time: cannot set cpu profile rate until previous profile has finished.\n")
			unlock(&cpuprofLock)
			return
		}
		...
		for {
			n := cpuprof.handoff
			if n&0x80000000 != 0 {
				print("run_time: setcpuprofile(off) twice\n")
			}
...
func (p *cpuProfile) getprofile() []byte {
	...
	if p.wholding {
		...
		for {
			n := p.handoff
			if n == 0 {
				print("run_time: phase error during cpu profile handoff\n")
				return nil
			}
			if n&0x80000000 != 0 {
	...
	switch n := p.handoff; {
	case n == 0:
		print("run_time: phase error during cpu profile wait\n")
		return nil
	case n == 0x80000000:
		p.flushing = true
		goto Flush
...
Flush:
	...
	if !cas(&p.handoff, p.handoff, 0) {
		print("run_time: profile flush racing with something\n")
	}
	return nil
}
...
//go:linkname run_time_pprof_run_time_cyclesPerSecond run_time/pprof.run_time_cyclesPerSecond
func run_time_pprof_run_time_cyclesPerSecond() int64 {
