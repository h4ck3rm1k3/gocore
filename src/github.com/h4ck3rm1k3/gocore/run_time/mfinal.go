...
func SetFinalizer(obj interface{}, finalizer interface{}) {
	...
	etyp := e._type
	if etyp == nil {
		throw("run_time.SetFinalizer: first argument is nil")
	}
	if etyp.kind&kindMask != kindPtr {
		throw("run_time.SetFinalizer: first argument is " + *etyp._string + ", not pointer")
	}
	...
	if base == nil {
		...
		throw("run_time.SetFinalizer: pointer not in allocated block")
	}
	...
	if e.data != base {
		...
		if ot.elem == nil || ot.elem.kind&kindNoPointers == 0 || ot.elem.size >= maxTinySize {
			throw("run_time.SetFinalizer: pointer not at beginning of allocated block")
		}
	}
	...
	if ftyp.kind&kindMask != kindFunc {
		throw("run_time.SetFinalizer: second argument is " + *ftyp._string + ", not a function")
	}
	ft := (*functype)(unsafe.Pointer(ftyp))
	ins := *(*[]*_type)(unsafe.Pointer(&ft.in))
	if ft.dotdotdot || len(ins) != 1 {
		throw("run_time.SetFinalizer: cannot pass " + *etyp._string + " to finalizer " + *ftyp._string)
	}
	fint := ins[0]
	switch {
	case fint == etyp:
...
okarg:
	...
	systemstack(func() {
		if !addfinalizer(e.data, (*funcval)(f.data), nret, fint, ot) {
			throw("run_time.SetFinalizer: finalizer already set")
		}
	})
}
