...
//extern SigTabTT run_time·sigtab[];
	...
	if !iscgo {
		if bsdthread_register() != 0 {
			if gogetenv("DYLD_INSERT_LIBRARIES") != "" {
				throw("run_time: bsdthread_register error (unset DYLD_INSERT_LIBRARIES)")
			}
			throw("run_time: bsdthread_register error")
		}
	}
}
...
func newosproc(mp *m, stk unsafe.Pointer) {
	...
	if errno < 0 {
		print("run_time: failed to create new OS thread (have ", mcount(), " already; errno=", -errno, ")\n")
...
func setsig(i int32, fn uintptr, restart bool) {
	...
	sa.sa_mask = ^uint32(0)
	sa.sa_tramp = unsafe.Pointer(funcPC(sigtramp)) // run_time·sigtramp's job is to call into real handler
