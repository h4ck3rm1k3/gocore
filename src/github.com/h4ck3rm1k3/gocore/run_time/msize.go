...
func initSizes() {
	// Initialize the run_time·class_to_size table (and choose class sizes in the process).
	class_to_size[0] = 0
	...
	if false {
		for n := int32(0); n < _MaxSmallSize; n++ {
			...
			if sizeclass < 1 || sizeclass >= _NumSizeClasses || class_to_size[sizeclass] < n {
				print("size=", n, " sizeclass=", sizeclass, " run_time·class_to_size=", class_to_size[sizeclass], "\n")
