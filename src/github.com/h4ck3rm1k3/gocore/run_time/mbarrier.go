...
func writebarrierptr_shadow(dst *uintptr, src uintptr) {
	systemstack(func() {
		...
		if *shadow != *dst && *shadow != noShadow && istrackedptr(*dst) {
			...
			print("run_time: write barrier dst=", dst, " old=", hex(*dst), " shadow=", shadow, " old=", hex(*shadow), " new=", hex(src), "\n")
...
func wbshadowinit() {
	...
	if ptrSize != 8 {
		print("run_time: GODEBUG=wbshadow=1 disabled on 32-bit system\n")
		return
	}
...
func checkwbshadow(p *uintptr) {
	...
	if *shadow != *p && *shadow != noShadow && istrackedptr(*p) {
		mheap_.shadow_enabled = false
		print("run_time: checkwritebarrier p=", p, " *p=", hex(*p), " shadow=", shadow, " *shadow=", hex(*shadow), "\n")
		throw("missed write barrier")
	}
}
