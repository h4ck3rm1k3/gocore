...
// Solaris run_time-integrated network poller.
//
...
// that eventually calls run_time·netpollopen. run_time·netpollopen
...
//
// Beside calling run_time·netpollopen, the networking code paths
// will call run_time·netpollarm each time goroutines are interested
// in doing network I/O. Because now we know what kind of I/O we
// are interested in (reading/writting), we can call port_associate
// passing the correct type of event set (POLLIN/POLLOUT). As we made
// sure to have already associated the file descriptor with the port,
// when we now call port_associate, we will unblock the main poller
// loop (in run_time·netpoll) right away if the socket is actually
// ready for I/O.
//
// The main poller loop runs in its own thread waiting for events
// using port_getn. When an event happens, it will tell the scheduler
// about it using run_time·netpollready. Besides doing this, it must
...
// run_time·netpollupdate. This function takes care to associate the
...
// association is effected only by the run_time·netpollarm code path,
...
// lock allows run_time·netpollupdate to be called synchronously from
...
retry:
	...
	if port_getn(portfd, &events[0], uint32(len(events)), &n, wait) < 0 {
		if e := errno(); e != _EINTR && e != netpolllasterr {
			netpolllasterr = e
			print("run_time: port_getn on fd ", portfd, " failed with ", e, "\n")
		}
		goto retry
	}
