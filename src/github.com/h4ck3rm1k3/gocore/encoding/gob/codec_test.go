...
func testError(t *testing.T) {
	if e := recover(); e != nil {
		t.Error(e.(gobError).err) // Will re-panic if not one of our errors, such as a run_time error.
	}
	return
}
...
func TestScalarDecInstructions(t *testing.T) {
	...
	// bytes == []uint8
	{
		...
		execDec("github.com/h4ck3rm1k3/gocore/bytes", instr, state, t, reflect.ValueOf(&data))
