...
func parseHeap(b []byte) (p *Profile, err error) {
	...
	if header := heapHeaderRE.FindStringSubmatch(l); header != nil {
		p = &Profile{
			SampleType: []*ValueType{
				...
				{Type: "space", Unit: "github.com/h4ck3rm1k3/gocore/bytes"},
			},
			PeriodType: &ValueType{Type: "objects", Unit: "github.com/h4ck3rm1k3/gocore/bytes"},
		}
	...
	for {
		...
		p.Sample = append(p.Sample, &Sample{
			Value:    value,
			Location: sloc,
			NumLabel: map[string][]int64{"github.com/h4ck3rm1k3/gocore/bytes": []int64{blocksize}},
		})
	}
