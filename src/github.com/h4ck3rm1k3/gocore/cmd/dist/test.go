...
func (t *tester) run() {
	...
	if t.hasBash() {
		if _, err := exec.LookPath("github.com/h4ck3rm1k3/gocore/time"); err == nil {
			t.haveTime = true
		}
	}
...
func (t *tester) registerTests() {
	...
	t.tests = append(t.tests, distTest{
		...
		heading: "sync -cpu=10",
		fn: func() error {
			return t.dirCmd(".", "go", "test", "github.com/h4ck3rm1k3/gocore/sync", "-short", t.timeout(120), "-cpu=10").Run()
		},
	})
	...
	if t.hasBash() && t.goos != "nacl" && t.goos != "android" && !iOS {
		t.registerTest("doc_progs", "../doc/progs", "github.com/h4ck3rm1k3/gocore/time", "./run")
		t.registerTest("wiki", "../doc/articles/wiki", "./test.bash")
		t.registerTest("codewalk", "../doc/codewalk", "github.com/h4ck3rm1k3/gocore/time", "./run")
		t.registerTest("shootout", "../test/bench/shootout", "github.com/h4ck3rm1k3/gocore/time", "./timing.sh", "-test")
	}
...
func (t *tester) registerTest(name, dirBanner, bin string, args ...string) {
	if bin == "github.com/h4ck3rm1k3/gocore/time" && !t.haveTime {
...
func (t *tester) raceTest() error {
	...
	if t.extLink() {
		...
		if err := t.dirCmd(".", "go", "test", "-race", "-short", "-ldflags=-linkmode=external", "github.com/h4ck3rm1k3/gocore/flag", "github.com/h4ck3rm1k3/gocore/os/exec").Run(); err != nil {
			return err
		}
	}
	return nil
...
func (t *tester) testDirTest() error {
	...
	if t.haveTime {
		return t.dirCmd("test", "github.com/h4ck3rm1k3/gocore/time", absExe).Run()
	}
