...
func synthesizemaptypes(die *DWDie) {
	...
	for ; die != nil; die = die.link {
		...
		// Construct hash<K,V>
		dwh = newdie(&dwtypes, DW_ABRV_STRUCTTYPE, mkinternaltypename("github.com/h4ck3rm1k3/gocore/hash", getattr(keytype, DW_AT_name).data.(string), getattr(valtype, DW_AT_name).data.(string)))
