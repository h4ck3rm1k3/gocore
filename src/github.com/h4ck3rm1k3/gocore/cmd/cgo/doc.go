...
The following options are available when running cgo directly:
	...
	-import_run_time_cgo
		If set (which it is by default) import run_time/cgo in
		generated output.
	-import_syscall
...
functions.  They all have similar bodies that invoke run_time·cgocall
to make a switch from the Go run_time world to the system C (GCC-based)
world.
...
For example, here is the definition of _Cfunc_puts:
	...
	func _Cfunc_puts(p0 *_Ctype_char) (r1 _Ctype_int) {
		_cgo_run_time_cgocall_errno(_cgo_be59f0f25121_Cfunc_puts, uintptr(unsafe.Pointer(&p0)))
		return
	}
...
threads and thread-local storage. The run_time package declares a few
variables:
...
Any package using cgo imports "github.com/h4ck3rm1k3/gocore/run_time/cgo", which provides
...
used to create a new thread, in place of the run_time's usual direct
system calls.
...
The following code will be generated by cgo:
	...
	func _Cfunc_sin(p0 _Ctype_double) (r1 _Ctype_double) {
		_cgo_run_time_cgocall_errno(_cgo_gcc_Cfunc_sin, uintptr(unsafe.Pointer(&p0)))
		return
	}
...
packages (net, os/user, run_time/cgo), 6l will use internal linking
